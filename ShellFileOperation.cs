...
namespace VideoCombine
{
    ...
    public class ShellFileOperation
    {
        ...
        /// <param name="cover">是否覆盖目标文件夹下同名文件</param>
        /// <returns>0表示成功，其余为错误码，可通过GetErrorString()方法获取错误内容</returns>
        public static int Copy(string sourceFile, string targetFile,bool cover)
        {
            return Copy(new string[] { sourceFile }, new string[] { targetFile }, cover);
        }
        ...
        /// <param name="cover">是否覆盖目标文件夹下同名文件</param>
        /// <returns>0表示成功，其余为错误码，可通过GetErrorString()方法获取错误内容</returns>
        public static int Copy(IEnumerable<string> sourceFiles, IEnumerable<string> targetFiles, bool cover = false)
        {
            return Copy(sourceFiles, targetFiles, false, cover);
        }
        ...
        /// <param name="cover">是否覆盖目标文件夹下同名文件</param>
        /// <returns>0表示成功，其余为错误码，可通过GetErrorString()方法获取错误内容</returns>
        private static int Copy(IEnumerable<string> sourceFiles, IEnumerable<string> targetFiles, bool deleteSourceFile,bool cover = false)
        {
            ...
            pm.fFlags = cover
                ? FILEOP_FLAGS.FOF_NOCONFIRMATION | FILEOP_FLAGS.FOF_MULTIDESTFILES | FILEOP_FLAGS.FOF_ALLOWUNDO
                : FILEOP_FLAGS.FOF_NOCONFIRMATION | FILEOP_FLAGS.FOF_MULTIDESTFILES | FILEOP_FLAGS.FOF_ALLOWUNDO |
        ...
        /// <param name="targetFiles">目标文件路径列表</param>
        /// <returns>0表示成功，其余为错误码，可通过GetErrorString()方法获取错误内容</returns>
        private static int Move(IEnumerable<string> sourceFiles, IEnumerable<string> targetFiles)
        {
        ...
        /// <returns>0表示成功，其余为错误码，可通过GetErrorString()方法获取错误内容</returns>
        private static int Delete(string filePath, bool allowUndo)
        {
